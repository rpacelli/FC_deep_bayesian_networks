...
mother_dir = f'./deepL_beyond_infwidth_runs/'
...
if not utils.os.path.isfile(theoryFilename):
	with open(theoryFilename,"a") as f:
		print('#1P', '2 N1', '3 Qbar', '4 pred error', 'yK^-1y/P', file = f)
if args.compute_theory:
	...
	with open(theoryFilename, "a") as f:
		print(args.P,args.N1,Qbar.item(), gen_error_pred, yky.item(),file = f)
...
#scheduler = StepLR(optimizer, step_size = args.checkpoint, gamma = args.gamma)               
...
test_with_train_args = [net,inputs,targets,test_criterion]
