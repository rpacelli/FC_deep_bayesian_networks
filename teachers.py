...
class mnist_dataset: 
    ...
    def make_data(self, P, P_test, device):
        ...
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=60000, num_workers=1)
        testset = torchvision.datasets.MNIST(root='./data', train=False, download=True, transform=transform_dataset)
        testloader = torch.utils.data.DataLoader(testset, batch_size=10000, num_workers=1)
...
class cifar_dataset: 
    ...
    def make_data(self, P, P_test, device):
        ...
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=600000, num_workers=1)
        testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform_dataset)
        testloader = torch.utils.data.DataLoader(testset, batch_size=60000, num_workers=1)
